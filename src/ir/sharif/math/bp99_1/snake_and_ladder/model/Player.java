...
public class Player {
    ...
    public Player(String name, int score, int id, int playerNumber) {
        ...
        this.pieces.add(new Healer(this, Color.RED));
        this.pieces.add(new Sniper(this, Color.BLUE));
        this.pieces.add(new Bomber(this, Color.GREEN));
        this.pieces.add(new Robber(this, Color.YELLOW));
        this.moveLeft = 0;
        this.moveRight = 0;
    ...
    public boolean hasMove(Board board, int diceNumber) {
        for (Piece i : pieces){
            if ((board.getCells().contains(board.getCell(i.getCurrentCell().getX() + diceNumber, i.getCurrentCell().getY()))) && i.isValidMove(board.getCell(i.getCurrentCell().getX() + diceNumber, i.getCurrentCell().getY()), diceNumber) ||
            (board.getCells().contains(board.getCell(i.getCurrentCell().getX(), i.getCurrentCell().getY() + diceNumber))) && i.isValidMove(board.getCell(i.getCurrentCell().getX(), i.getCurrentCell().getY() + diceNumber), diceNumber) ||
            (board.getCells().contains(board.getCell(i.getCurrentCell().getX() - diceNumber, i.getCurrentCell().getY()))) && i.isValidMove(board.getCell(i.getCurrentCell().getX() - diceNumber, i.getCurrentCell().getY()), diceNumber) ||
            (board.getCells().contains(board.getCell(i.getCurrentCell().getX(), i.getCurrentCell().getY() - diceNumber))) && i.isValidMove(board.getCell(i.getCurrentCell().getX(), i.getCurrentCell().getY() - diceNumber), diceNumber)){
                return true;
            }
        }
